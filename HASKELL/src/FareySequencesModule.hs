  ...
  | otherwise =
    if successor == 1 % 1
      then predecessorOfOneFirstInFm m
                 ...
                 ceiling
                   (toRational (numerator successor * m) /
                    toRational (denominator successor))
            in getNumeratorAndReturnPredecessor
  ...
  | otherwise =
    if predecessor == 0 % 1
      then successorOfZeroFirstInFm m
                 ...
                 ceiling
                   (toRational (numerator predecessor * m + 2) /
                    toRational (denominator predecessor))
            in getNumeratorAndReturnSuccessor
                 (findNumeratorOfSuccessor
                    (refPoint - numerator predecessor, refPoint - 1)
                    predecessor)
                 predecessor
...
predecessorInFml m l successor
  ...
  | otherwise =
    if successor == 1 % 1
      ...
      else if numerator successor * m - denominator successor * l >= 1
             then getNumeratorAndReturnPredecessor
                    (findNumeratorOfPredecessor
                       (l - numerator successor + 1, l)
                       successor)
                    successor
             else let refPoint =
                        ceiling
...
successorInFml m l predecessor
  ...
  | otherwise =
    if predecessor == 0 % 1
      then successorOfZeroFirstInFml m
      else if denominator predecessor * l - numerator predecessor * m >= 1
             then let refPoint =
                        ceiling
                          (toRational (numerator predecessor * m + 2) /
                           toRational (denominator predecessor))
                   in getNumeratorAndReturnSuccessor
                        (findNumeratorOfSuccessor
                           ...
                           predecessor)
                        predecessor
             else getNumeratorAndReturnSuccessor
                    (findNumeratorOfSuccessor
                       (l - numerator predecessor + 1, l)
                       predecessor)
                    predecessor
      ...
      then predecessorOfOneFirstInGml m
             ...
             then let refPoint =
                   ...
                   in getNumeratorAndReturnPredecessor
                        ...
                        ceiling
                          (toRational (numerator successor * (m - l)) /
                           toRational
  ...
  | otherwise =
    if predecessor == 0 % 1
      then successorOfZeroFirstInGml m l
             ...
             then let refPoint =
                        ceiling
                          (toRational (numerator predecessor * (m - l) + 2) /
                           toRational
...
predecessorInFB2mm m successor
  ...
  | otherwise =
    if | successor == 1 % 1 -> predecessorOfOneFirstInFB2mm m
           ...
           then (if (successor > 1 % 2) &&
                   ...
                   then getNumeratorAndReturnPredecessor
                          ...
                          successor
                               ...
                               ceiling
                                 (toRational (numerator successor * (n - m)) /
                                  toRational
                                    (denominator successor - numerator successor))
                          in getNumeratorAndReturnPredecessor
                               (findNumeratorOfPredecessor
...
successorInFBnm n m predecessor
  ...
  | otherwise =
    if | predecessor == 0 % 1 -> successorOfZeroFirstInFBnm n m
       ...
       | otherwise ->
         if n < 2 * m
           then (if (predecessor > 1 % 2) &&
                    (denominator predecessor * m - numerator predecessor * n <=
                     1)
                   then getNumeratorAndReturnSuccessor
                          (findNumeratorOfSuccessor
                             (m - numerator predecessor + 1, m)
                             predecessor)
                          predecessor
                               ...
                               ceiling
                                 (toRational
                                    (numerator predecessor * (n - m) + 2) /
                                  toRational
                                    (denominator predecessor -
                                     numerator predecessor))
                          in getNumeratorAndReturnSuccessor
                               (findNumeratorOfSuccessor
                                  ( refPoint - numerator predecessor
                                  , refPoint - 1)
                                  predecessor)
                               predecessor))
...
getNumeratorAndReturnPredecessor a successor =
  a % ((denominator successor * a + 1) `div` numerator successor)

getNumeratorAndReturnSuccessor :: Integer -> Rational -> Rational
getNumeratorAndReturnSuccessor a predecessor =
  a % ((denominator predecessor * a - 1) `div` numerator predecessor)
...
findNumeratorOfPredecessor searchInterval successor =
  fromMaybe
    0
    (find
       (\x -> (denominator successor * x + 1) `mod` numerator successor == 0)
       [fst searchInterval .. snd searchInterval])
...
findNumeratorOfSuccessor searchInterval predecessor =
  fromMaybe
    0
    (find
       (\x -> (denominator predecessor * x - 1) `mod` numerator predecessor == 0)
       [fst searchInterval .. snd searchInterval])
...
successorOfZeroFirstInFml :: Integer -> Rational
-- See Remark 1.9 and Table 1.5  of the monograph
...
predecessorOfOneFirstInFBnm :: Integer -> Rational
-- See Remark 1.17 and Table 1.5 of the monograph
predecessorOfOneFirstInFBnm m = m % (m + 1)
...
predecessorOfPairOfNeighborsInFm ::
     Integer -> (Rational, Rational) -> Bool -> Rational
...
predecessorOfPairOfNeighborsInFm m (successor, rightNeighborOfSuccessor) checkPair
  ...
  | not checkPair ||
      (checkPair && (successor == predecessorInFm m rightNeighborOfSuccessor)) =
    let fareyIndex =
          floor
            (toRational (m + denominator rightNeighborOfSuccessor) /
             toRational (denominator successor))
     in (fareyIndex * numerator successor - numerator rightNeighborOfSuccessor) %
        (fareyIndex * denominator successor -
         denominator rightNeighborOfSuccessor)
...
successorOfPairOfNeighborsInFm m (leftNeighborOfPredecessor, predecessor) checkPair
  ...
  | not checkPair ||
      checkPair && predecessor == successorInFm m leftNeighborOfPredecessor =
    let fareyIndex =
          floor
            (toRational (m + denominator leftNeighborOfPredecessor) /
             toRational (denominator predecessor))
     in (fareyIndex * numerator predecessor -
         numerator leftNeighborOfPredecessor) %
        (fareyIndex * denominator predecessor -
         denominator leftNeighborOfPredecessor)
  | otherwise = -1 % 7 -- "N/A: The input pair is not a pair of neighboring fractions in this Farey sequence"

predecessorOfPairOfNeighborsInFml ::
     Integer -> Integer -> (Rational, Rational) -> Bool -> Rational
...
predecessorOfPairOfNeighborsInFml m l (successor, rightNeighborOfSuccessor) checkPair
  ...
  | not checkPair ||
      checkPair && successor == predecessorInFml m l rightNeighborOfSuccessor =
    if numerator successor * m - denominator successor * l >= 1
      then let fareyIndex =
                 floor
                   (toRational (l + numerator rightNeighborOfSuccessor) /
                    toRational (numerator successor))
            in (fareyIndex * numerator successor -
                numerator rightNeighborOfSuccessor) %
               (fareyIndex * denominator successor -
                denominator rightNeighborOfSuccessor)
           -- numerator successor * m - denominator successor * l < 1
      else let fareyIndex =
                 floor
                   (toRational (m + denominator rightNeighborOfSuccessor) /
                    toRational (denominator successor))
...
successorOfPairOfNeighborsInFml m l (leftNeighborOfPredecessor, predecessor) checkPair
  ...
  | not checkPair ||
      checkPair && predecessor == successorInFml m l leftNeighborOfPredecessor =
    if denominator predecessor * l - numerator predecessor * m >= 1
      then let fareyIndex =
            ...
            in (fareyIndex * numerator predecessor -
               ...
               (fareyIndex * denominator predecessor -
                denominator leftNeighborOfPredecessor)
           -- denominator predecessor * l - denominator predecessor * m  < 1
      else let fareyIndex =
                 floor
                   (toRational (l + numerator leftNeighborOfPredecessor) /
                    toRational (numerator predecessor))
            in (fareyIndex * numerator predecessor -
                numerator leftNeighborOfPredecessor) %
               (fareyIndex * denominator predecessor -
                denominator leftNeighborOfPredecessor)
    ...
    if numerator successor * m - denominator successor * l >= 1
      then let fareyIndex =
           ...
           -- numerator successor * m - denominator successor * l < 1
                 ...
                 floor
                   (toRational
                      (m - l + denominator rightNeighborOfSuccessor -
                       numerator rightNeighborOfSuccessor) /
                    toRational (denominator successor - numerator successor))
            in (fareyIndex * numerator successor -
                numerator rightNeighborOfSuccessor) %
               (fareyIndex * denominator successor -
                denominator rightNeighborOfSuccessor)
  ...
  | not checkPair ||
    ...
    if denominator predecessor * l - numerator predecessor * m >= 1
      then let fareyIndex =
                 floor
                   (toRational
                      (m - l + denominator leftNeighborOfPredecessor -
                       numerator leftNeighborOfPredecessor) /
                    toRational (denominator predecessor - numerator predecessor))
            in (fareyIndex * numerator predecessor -
                numerator leftNeighborOfPredecessor) %
               (fareyIndex * denominator predecessor -
                denominator leftNeighborOfPredecessor)
           -- denominator predecessor * l - numerator predecessor * m < 1
      else let fareyIndex =
                 floor
                   (toRational (m + denominator leftNeighborOfPredecessor) /
                    toRational (denominator predecessor))
...
predecessorOfPairOfNeighborsInFBnm n m (successor, rightNeighborOfSuccessor) checkPair
  ...
  | not checkPair ||
    ...
    if numerator successor * n - denominator successor * m >= 1
      then let fareyIndex =
                 floor
                   (toRational (m + numerator rightNeighborOfSuccessor) /
                    toRational (numerator successor))
            in (fareyIndex * numerator successor -
                numerator rightNeighborOfSuccessor) %
               (fareyIndex * denominator successor -
           ...
           -- numerator successor * n - denominator successor * m < 1
                 ...
                 floor
                   (toRational
                      (n - m + denominator rightNeighborOfSuccessor -
                       numerator rightNeighborOfSuccessor) /
                    toRational (denominator successor - numerator successor))
            in (fareyIndex * numerator successor -
                numerator rightNeighborOfSuccessor) %
...
predecessorOfPairOfNeighborsInFB2mm m (successor, rightNeighborOfSuccessor) checkPair =
  predecessorOfPairOfNeighborsInFBnm
    (2 * m)
    m
    (successor, rightNeighborOfSuccessor)
    checkPair
  ...
  | not checkPair ||
    ...
    if denominator predecessor * m - numerator predecessor * n >= 1
      then let fareyIndex =
                 floor
                   (toRational
                      (n - m + denominator leftNeighborOfPredecessor -
                       numerator leftNeighborOfPredecessor) /
                    toRational (denominator predecessor - numerator predecessor))
            in (fareyIndex * numerator predecessor -
                numerator leftNeighborOfPredecessor) %
               (fareyIndex * denominator predecessor -
                denominator leftNeighborOfPredecessor)
           -- denominator predecessor * m - numerator predecessor * n < 1
      else let fareyIndex =
                 floor
                   (toRational (m + numerator leftNeighborOfPredecessor) /
                    toRational (numerator predecessor))
            in (fareyIndex * numerator predecessor -
                numerator leftNeighborOfPredecessor) %
               (fareyIndex * denominator predecessor -
...
successorOfPairOfNeighborsInFB2mm m (leftNeighborOfPredecessor, predecessor) checkPair =
  successorOfPairOfNeighborsInFBnm
    (2 * m)
    m
    (leftNeighborOfPredecessor, predecessor)
    checkPair
